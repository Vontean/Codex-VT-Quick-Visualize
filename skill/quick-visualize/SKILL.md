...
3. Write a UTF-8 JSON spec in `work/` or another safe temporary directory. Finish when every required value and interaction is explicit and valid under the selected reference.
4. Resolve this skill directory and run `python3 <renderer> <spec.json> <thread-visualization-directory>/<title>.html`. Finish when the renderer exits successfully and returns the requested fragment path.
5. Read the fragment once. Finish when it matches the spec, contains no unresolved `{{...}}` token, and its JavaScript parses.
6. Put any necessary question or explanation in normal Markdown, then emit the Visualize content reference with the renderer's exact output path. Resolve it from the current task environment; never hard-code a developer-machine directory. Finish when the reference points to the fragment generated in this invocation.
